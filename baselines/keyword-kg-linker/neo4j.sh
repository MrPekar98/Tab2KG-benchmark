...
then
  ...
  docker run -d -p ${PORT}:${PORT} --network linker-dev -p ${PORT_BULP}:${PORT_BULP} --name ${CONTAINER_NAME} -v ${PWD}/${KG_DIR}:/kg \
      -e NEO4J_AUTH=none \
  ...
  sleep 1m
  docker exec ${CONTAINER_NAME} wget -P plugins/ https://github.com/neo4j-labs/neosemantics/releases/download/4.1.0.1/neosemantics-4.1.0.1.jar
  docker exec ${CONTAINER_NAME} bash -c "echo 'dbms.unmanaged_extension_classes=n10s.endpoint=/rdf' >> conf/neo4j.conf"
  docker restart ${CONTAINER_NAME}

  echo "Importing knowledge graph..."
  sleep 1m
  docker cp load.sh ${CONTAINER_NAME}:/var/lib/neo4j
  docker exec ${CONTAINER_NAME} bash -c "./load.sh ${NEO4J_HOME} ${NEO4J_IMPORT}"
